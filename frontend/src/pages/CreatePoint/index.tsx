...
{ 
  ...
  function handleSelectItem(item_id: number){
    const alreadySelected = selectedItem.findIndex(item => item === item_id);

    if(alreadySelected >= 0){
      const filteredItems = selectedItem.filter(item => item !== item_id);

      setSelectedItem(filteredItems);
    }else{
      setSelectedItem([ ...selectedItem, item_id]);
    }   
  ...
  async function handleSubmit(event: FormEvent){
    ...
    try{
      ...
      alert('Falha ao cadastrar ponto ! ')
    }
